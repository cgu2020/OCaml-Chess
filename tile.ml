...
let empty_tile = { piece = Empty; color = None; position = (0, 0) }

let pawn = { piece = Pawn; color = White; position = (0, 0) }

let check_capital (s : string) : bool =
  if String.equal (String.uppercase_ascii s) s then true else false
