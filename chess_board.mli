...
(** [possible_moves a] is a set-like list of all the tiles 
    that a can move to.*)
val possible_moves : Tile.tile -> Tile.tile array array -> (int * int) list
...
(** [move_piece b x1 y1 x2 y2 c] clears tile (x1,y1) and replaces tile (x2,y2) with tile (x1, y1). 
Invalid move if the int c does not match the color
